// ============================================================================
// KLAMBOT.RU - Status Aliases Parser
...
const STATUS_ALIASES: Record<string, string> = {
  ...
  'замечания': 'remarks',
  'замечание': 'remarks',
  'замечания есть': 'remarks',
  'есть замечания': 'remarks',
  'есть правки': 'remarks',
  'доработка': 'remarks',
  'доработать': 'remarks',
  'доработай': 'remarks',
  'нужна доработка': 'remarks',
  'исправить': 'remarks',
  'исправь': 'remarks',
  'исправления': 'remarks',
  'нужны исправления': 'remarks',
  'переделать': 'remarks',
  'переделай': 'remarks',
  'правки': 'remarks',
  'правка': 'remarks',
  'внести правки': 'remarks',
 ...
 * Поддерживаются форматы: АР-001, АР001, КР1, OVVK-123, КР-1, АР-12, ГП, АС
 */
const ALBUM_CODE_PATTERN = /(?:^|[^\wА-Яа-я])([А-ЯA-Z]{2,4}-?\d{0,4})(?:[^\wА-Яа-я]|$)/gi;
...
export function parseStatusCommands(text: string, validAlbumCodes?: string[]): StatusChangeCommand[] {
  ...
  for (const albumCode of foundAlbumCodes) {
    ...
    for (const [alias, statusCode] of Object.entries(STATUS_ALIASES)) {
      ...
      const aliasPattern = new RegExp(`\\b${escapedAlias}\\b|${escapedAlias}`, 'i');
